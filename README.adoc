...
----
docker build -t james-gatling-runner .
----

You can change the configuration of the Gatling project before the build, so that maybe it targets tour environment
with the list of users you want to use for example.

To launch it:

----
docker run -it --rm james-gatling-runner
----
