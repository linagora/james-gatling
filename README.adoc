...
.scala
----
org.apache.james.gatling.jmap.scenari.JmapScenario
----

Authenticate 100 JMAP users, one time. Then they all retrieve their system mailboxes. Then they send each other 10 mails each at random. Then wait 60 seconds for mails to be delivered.

They then perform the given actions :

 - 25% : Reads a message randomly
 - 20% : List messages
 - 20% : Sends a message rendomly
 - 15% : Marks a message as read
 - 10% : Retrieve its mailbox list
 -  5% : Mark a message as answered
 -  5% : Mark a message as flagged