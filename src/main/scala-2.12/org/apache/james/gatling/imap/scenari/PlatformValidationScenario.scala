...
class PlatformValidationScenario(minWaitDelay: Duration = 2 seconds, maxWaitDelay: Duration = 18 seconds)  {
  ...
  val initialConnection: ChainBuilder = group("initialConnection")(
    exec(imap("Connect").connect()).exitHereIfFailed
      .exec(imap("capability1").capability().check(ok))
      .exec(imap("login").login("${username}", "${password}").check(ok))
      .exec(imap("capability2").capability().check(ok))
      ...
      .exec(imap("flagResync").fetch(MessageRanges(Range(1, 1000000)), AttributeList("FLAGS"))))
  ...
  val coreActions: ChainBuilder = group("coreActions")(
    randomSwitch(
      ...
      6.0 -> exec(imap("fetchFlags").uidFetch(MessageRanges(Range(1, 100000)), AttributeList("UID", "FLAGS")).check(ok)),
      6.0 -> exec(imap("fetchHeaders").uidFetch(MessageRanges(Last()), AttributeList("UID", "RFC822.SIZE", "FLAGS", "BODY.PEEK[HEADER.FIELDS (From To Cc Bcc Subject Date Message-ID Priority X-Priority References Newsgroups In-Reply-To Content-Type Reply-To)]")).check(ok)),
      6.0 -> exec(imap("fetchBody").uidFetch(MessageRanges(Last()), AttributeList("UID", "RFC822.SIZE", "BODY.PEEK[]")).check(ok)),
      2.0 -> exec(imap("searchUid").search(MessageRanges(Range(1, 100000)), null).check(ok)),
      2.0 -> exec(imap("searchUnseen").search(MessageRanges(Range(1, 100000)), new FlagTerm(new Flags(Flags.Flag.SEEN), false)).check(ok)),
      ...
      1.0 -> exec(imap("unselect").unselect().check(ok))
        .exec(imap("select2").select("INBOX").check(ok)),
